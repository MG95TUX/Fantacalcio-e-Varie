...
public Championstest(String sq, int po, double va, int ch){
squadra= sq;
posizione =po;
valore=va;
checkvalue=ch;
...
public int getcheckvalue(){return checkvalue;}
   ...
   Championstest array[] = new Championstest[8];
   int i=0;
   while (i< 8){
       int s=i+1;
       System.out.println("dammi la squadra N°" + s);
       String team= in.next();
       System.out.println("dimmi se " + team + "(squadra N°" + s +") era 1 o 2");
       int place= in.nextInt();
       if ((place != 1) && (place !=2)) throw new IllegalArgumentException();
       double value = Math.random();
       System.out.println("dimmi in che girone " + team + " (squadra N°" + s +") era (1 se a, 2 se b, 3 se c, 4 se b)");
       int check=in.nextInt();
       if ((check < 1) && (check > 4)) throw new IllegalArgumentException();
       if (place == 1){ value = value + 1;};           //evitare primi vs primi
       array[i] = new Championstest(team, place, value, check);
       team =null;
       place=0;
       value=0;
       check=0;
...
int l=0;
while (l <8){
String temp1=   array[l].getsquadra() ;
System.out.print(temp1 + " ");
l++;}
int q=0;
while (q <8){
double temp1=   array[q].getvalore() ;
System.out.print(temp1 + " ");
q++;}
}
}
